...
def WNexp(state, channel):
    ...
    for epoch_count in range(1, state.totepochs + 1):
        ...
        for i in range(state.nbatches):
            tmpl = trainl[:, int(i * batchsize):int((i + 1) * batchsize)]
            tmpr = trainr[:, int(i * batchsize):int((i + 1) * batchsize)]
            tmpo = traino[:, int(i * batchsize):int((i + 1) * batchsize)]
            tmpnl = trainln[:, int(i * batchsize):int((i + 1) * batchsize)]
            tmpnr = trainrn[:, int(i * batchsize):int((i + 1) * batchsize)]
            # training iteration
